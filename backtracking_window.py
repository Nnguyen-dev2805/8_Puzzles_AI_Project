...
class BacktrackingWindow:
    ...
    def run_ac3(self):
        """Chạy thuật toán AC-3 và cập nhật CSP."""
        if not self.csp:
            initial_state = [[None for _ in range(3)] for _ in range(3)]
            flat_state = [num if num is not None else 0 for row in initial_state for num in row]
            variables = [f"X{i+1}" for i in range(9)]
        ...
        new_domains = self.ac3(self.csp)
        if new_domains is None:
            self.step_info_label.config(text="AC-3: Không có giải pháp!")
            messagebox.showinfo("Thông báo", "AC-3: Không có giải pháp!")
        else:
            self.csp['domains'] = new_domains
            domains_info = "\n".join([f"{var}: {values}" for var, values in new_domains.items()])
            self.step_info_label.config(text="AC-3 hoàn thành! Miền mới:\n" + domains_info)
            messagebox.showinfo("Thông báo", "AC-3 hoàn thành! Miền đã được thu hẹp.")
            # Chạy lại backtracking với miền mới
            result = self.solve([[None for _ in range(3)] for _ in range(3)])
            if result['solution']:
                self.backtracking_steps = result['path']
                self.update_backtracking_board(self.backtracking_steps[0], 0)
                self.step_button.config(state=tk.NORMAL)
                self.auto_button.config(state=tk.NORMAL)
                self.step_info_label.config(text=f"AC-3 + Backtracking: Tìm thấy giải pháp! Nodes: {result['nodes_expanded']}")
            else:
    ...
    def show_csp(self):
        """Hiển thị thông tin CSP hiện tại."""
        if not self.csp:
            initial_state = [[None for _ in range(3)] for _ in range(3)]
            flat_state = [num if num is not None else 0 for row in initial_state for num in row]
            variables = [f"X{i+1}" for i in range(9)]
        ...
        domains_info = "\n".join([f"{var}: {values}" for var, values in domains.items()])
        constraints_info = "\n".join([
            "Không trùng lặp" if callable(c) else f"{c[0]}: {c[1].__code__.co_code.hex()}"
            for c in constraints
        ])
        
        csp_info = f"Biến:\n{', '.join(variables)}\n\nMiền:\n{domains_info}\n\nRàng buộc:\n{constraints_info}"
        self.step_info_label.config(text="Thông tin CSP:\n" + csp_info)
        messagebox.showinfo("Thông tin CSP", csp_info)
    ...
    def run_test(self):
        ...
        self.step_count_label.config(text="Số bước: 0")

        visited = set()
        path = []
        max_steps = 100  # Giảm giới hạn để tăng tốc
        ...
        while len(path) < max_steps:
            ...
            inversions = self.count_inversions(assignment)
            self.step_info.append(f"Thử trạng thái: {new_state}, Hoán vị: {inversions}")
            
            # Cập nhật số bước
            self.step_count_label.config(text=f"Số bước: {len(path)}")
            
            if not self.is_violate_constrain(new_state):
                self.backtracking_steps = path
                self.update_backtracking_board(self.backtracking_steps[0], 0)
                self.step_button.config(state=tk.NORMAL)
                self.auto_button.config(state=tk.NORMAL)
                self.step_info_label.config(text="TEST: Đã tìm thấy mục tiêu!")
                messagebox.showinfo("Kết quả kiểm tra", f"Đã tìm thấy mục tiêu sau {len(path)} bước!")
                break
            ...
            if len(path) >= max_steps:
                ...
                self.step_button.config(state=tk.NORMAL)
                self.auto_button.config(state=tk.NORMAL)
                self.step_info_label.config(text=f"TEST: Không tìm thấy mục tiêu sau {max_steps} bước!")
                messagebox.showinfo("Kết quả kiểm tra", f"Không tìm thấy mục tiêu sau {max_steps} bước!")
                break
        # """Chạy kiểm tra với các trạng thái ngẫu nhiên cho đến khi đạt mục tiêu."""
        # self.backtracking_steps = []
        # self.step_info = []
        # self.current_step = 0
        # self.step_info_label.config(text="Đang chạy kiểm tra TEST...")
        ...
        # while True:
        #     new_state = self.generate_random_state()
        #     state_tuple = tuple(tuple(row) for row in new_state)  # Chuyển thành tuple để lưu vào set
        #     if state_tuple in visited:
        #         continue
            
        #     visited.add(state_tuple)
        #     path.append(new_state)
        #     self.step_info.append(f"Thử trạng thái: {new_state}")
        ...
        #     # Giới hạn số bước tối đa để tránh vòng lặp vô hạn
        #     if len(path) >= 1000:
        #         self.backtracking_steps = path
        #         self.update_backtracking_board(self.backtracking_steps[0], 0)
        #         self.step_button.config(state=tk.NORMAL)
        #         self.auto_button.config(state=tk.NORMAL)
        #         self.step_info_label.config(text="TEST: Không tìm thấy mục tiêu sau 1000 bước!")
        #         messagebox.showinfo("Kết quả kiểm tra", "Không tìm thấy mục tiêu sau 1000 bước!")
        #         break
    ...
    def generate_random_state(self):
        ...
        while True:
            ...
            state = [row[:] for row in self.goal_state]
            ...
            if inversions % 2 == 0:
                return state
        # """Tạo trạng thái ngẫu nhiên cho bài toán 8-puzzle."""
        # numbers = list(range(9))  # [0, 1, 2, ..., 8]
        # random.shuffle(numbers)
        # return [numbers[i:i+3] for i in range(0, 9, 3)]  # Trả về danh sách 2D

    def is_violate_constrain(self, state):
